<template>
  ...
  <span
    @click="
      () => {
        router.push('/products/basket');
      }
    "
    class="flex fixed bottom-[10%] w-12 h-12 rounded-full animate-pulse z-50 bg-lime-500 right-5 items-center justify-center gap-2 cursor-pointer"
  >
    <svg
  ...
  <section class="">
    <div class="container flex flex-col justify-center">
      <div
        class="top font-bold border-b-2 flex flex-col gap-2 md:flex-row items-center justify-between py-6 text-center"
      >
        <div class="left w-[20%] text-start">
          <h1 class="text-center text-3xl">Products</h1>
        </div>
        <div
          class="right flex gap-2 items-center justify-between px-1 w-full md:w-[80%]"
        >
          <div class="md:w-1/2">
              ...
              <div class="flex">
                <div class="relative">
                  <input
                    ...
                    v-model="searchQuery"
                    id="search-dropdown"
                    class="block p-2.5 w-full z-20 text-sm text-gray-900 bg-gray-50 rounded-lg border-s-2 border border-gray-300 focus:ring-lime-500 focus:border-lime-500 dark:bg-gray-700 dark:border-s-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:border-lime-500"
                    placeholder="Search Mockups, Logos, Design Templates..."
                    required
                  />
                  <button
                    type="button"
                    class="absolute top-0 end-0 p-2.5 text-sm font-medium h-full text-white bg-lime-700 rounded-e-lg border border-lime-700 hover:bg-lime-800 focus:ring-4 focus:outline-none focus:ring-lime-300 dark:bg-lime-600 dark:hover:bg-lime-700 dark:focus:ring-lime-800"
                  >
                    <svg
                      class="w-4 h-4"
                      aria-hidden="true"
      ...
      <div class="flex md:flex-row flex-col">
        <div
          class="filter md:flex flex-col hidden items-center gap-3 py-3 border-r-2 pr-3 w-[20%]"
        >
          ...
          <div class="flex md:flex-col flex-wrap">
            ...
            >
              <span class="flex w-full p-2 items-center justify-center gap-3">
                ...
                <div class="right w-1/2">
                  <h1 class="font-sans font-medium text-base">
                    Insektitsidlar
                  </h1>
                </div>
              </span>
          ...
          >
            ...
            <div class="px-5 pb-5">
              ...
              <div class="flex flex-wrap items-center mt-2.5 mb-2">
                <h3
                  class="text-lime-500 underline cursor-pointer dark:text-white"
                >
                  Batafsil
                </h3>
              </div>
              <div class="flex items-center justify-between">
                ...
                <span class="flex items-center justify-center gap-2">
                  <button
                    ...
                    class="w-8 h-8 text-center bg-lime-500 rounded-full text-white font-bold text-xl"
                  >
                    -
                  </button>
                  <span>
                    {{
                    buyProducts.find((product) => product.id === item.id)
                      ? buyProducts.find((product) => product.id === item.id).count
                      : 0
                  }}
                  </span>
                  <button
...
function countBasket() {
  counts.value = localStorage.getItem("buyProducts")
   ? JSON.parse(localStorage.getItem("buyProducts")).length
    : 0;
}
countBasket()
...
const add = (id) => {
  ...
  if (item) {
    item.count++;
    const existingProductIndex = buyProducts.value.findIndex(
      (product) => product.id === id
    );

    if (existingProductIndex !== -1) {
      buyProducts.value[existingProductIndex] = item;
    } else {
      buyProducts.value.push(item);
    }
  }
...
function checked() {
  for (let i = 0; i < buyProducts.value.length; i++) {
    if (buyProducts.value[i].count === 0) {
      buyProducts.value.splice(i, 1);
      i--;
    }
  }
}
checked();
const remove = (id) => {
  const item = buyProducts.value.find((product) => product.id === id);
  if (item) {
    if (item.count > 0) {
      item.count--;
    } else {
